...
def export_to_gexf(corpus_path: str, output_path: str, threshold: float = 0.1, mode: str = 'semantic'):
    ...
    Args:
        corpus_path: Path to the corpus directory (containing corpus.db)
        output_path: Path to save the .gexf file
        threshold: Proportional threshold (0.0 to 1.0). 
                   Edges must be supported by at least (threshold * total_papers) papers.
        mode: Visualization mode ('semantic', 'authors', 'concepts', 'river', 'similarity')
    """
    ...
    with get_db_session(corpus_path) as session:
        ...
        if total_papers == 0:
            logger.warning("No papers in corpus. Exporting empty graph.")
            min_weight = 1
        else:
        ...
        # 4. Filter Nodes (Only keep nodes that have edges)
        active_node_ids = set()
        for edge in edges:
        ...
        # Most new modes are undirected (co-occurrence, similarity)
